...
vec3 getNormal(vec3 pos) {
  ...
  const defines: Record<string, any> = { ...((physical.defines ?? {}) as Record<string, any>) };
  defaults.dispose();
...
export default function Beams({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (
        lastConfig.beamNumber !== beamNumber ||
        lastConfig.beamWidth !== beamWidth ||
        lastConfig.beamHeight !== beamHeight
      ) {
        group.remove(mesh);
        geometry.dispose();
  ...
  return <div ref={containerRef} className={`absolute inset-0 h-full w-full ${className}`}></div>;
}
